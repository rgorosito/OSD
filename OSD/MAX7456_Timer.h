...
uint32_t prevTime = 0;           // previous time since start when OSD.update() ran
uint16_t prevArmedTimeSecs = 111; // bogus to force update
uint32_t armedTime = 0;          // time motors have spent armed

void displayFlightTime() {
  if (osd_info.motorsArmed ) {
    armedTime += ( osd_info.currentTime-prevTime );
  }

  prevTime = osd_info.currentTime;
  unsigned int armedTimeSecs = armedTime / 1000;

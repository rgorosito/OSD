...
void displayAltitude() {
  #ifdef USUnits
    int currentAltitude = osd_info.readedAltitude*3.281;
    int currentHoldAltitude = osd_info.desiredAltitudeToKeep*3.281;
  #else // metric
    int currentAltitude = osd_info.readedAltitude*10.0; // 0.1m accuracy!!
    int currentHoldAltitude = osd_info.desiredAltitudeToKeep*10.0;
  #endif
  char buf[7];
  ...
  switch (osd_info.altHoldState) {
  case false:
    if (lastHoldState != false) {
      lastHoldState = false;
      memset(buf,0,6);
      isWriteNeeded = true;
    }
    break;
  case true:
    if ((lastHoldState != true) || (lastHoldAltitude != currentHoldAltitude)) {
      lastHoldState = true;

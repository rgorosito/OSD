...
void displayArtificialHorizon() {
  ...
  for (byte i=0; i<6; i++) {
    byte row = constrain(
			 AH_CENTRE + 
			 (14.5 - (float)AH_COLUMNS[i]) * 12 * 1.4 * osd_info.roll +
			 (osd_info.pitch/AH_MAX_PITCH_ANGLE*(AH_CENTRE-AH_TOP_PIXEL)),
  ...
  if (lastFlightMode != osd_info.attitude) {
    writeChars( ! osd_info.attitude ? "\1\2" : "\3\4", 2, 0, RETICLE_ROW, RETICLE_COL ); //write 2 chars to row (middle), column 14
    lastFlightMode = osd_info.attitude;
  }
}
